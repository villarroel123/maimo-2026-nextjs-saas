...
function serializeFavorite(doc) {
  ...
  return {
    ...
    concertCountry: data.concertCountry || "",
...
async function getFavoriteData(target) {
  ...
  return {
    ...
    concertDate: project["Dia del concierto"] || "",
    concertCountry: project.Pais || "",
  };
}
...
export async function getFavoriteAgendaForUser(uid) {
  const favorites = await getFavoritesForUser(uid);
  const projectIds = [...new Set(favorites.map((favorite) => favorite.projectId).filter(isValidId))];
  const projectSnapshots = await Promise.all(
    projectIds.map(async (projectId) => [
      projectId,
      await getDb().collection(PROJECTS_COLLECTION).doc(projectId).get(),
    ]),
  );
  const projects = new Map(
    projectSnapshots
      .filter(([, snapshot]) => snapshot.exists)
      .map(([projectId, snapshot]) => [projectId, snapshot.data()]),
  );

  return favorites
    .filter((favorite) => favorite.type === "fanproject")
    .map((favorite) => {
      const project = projects.get(favorite.projectId);

      return {
        ...favorite,
        concertTitle: favorite.concertTitle || project?.Titulo || favorite.description || "Concierto",
        concertDate: favorite.concertDate || project?.["Dia del concierto"] || "",
        concertCountry: favorite.concertCountry || project?.Pais || "",
      };
    });
}
